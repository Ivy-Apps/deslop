...
spec = describe "getSecrets" $ do
    ...
    it "invalid secrets" $ do
        res <- runGetSecrets (fixturesPath </> [osp|secrets|] </> [osp|invalid.json|])
        res `shouldSatisfy` \case
            Left (InvalidSecretsJson _) -> True
            _ -> False
    it "vaid secrets without a GeminiApiKey" $ do
        res <- runGetSecrets (fixturesPath </> [osp|secrets|] </> [osp|no-gemini-api-key.json|])
        let expected = Secrets {geminiApiKey = Nothing}
        res `shouldBe` Right expected
    it "valid secrets" $ do
        res <- runGetSecrets (fixturesPath </> [osp|secrets|] </> [osp|valid.json|])
