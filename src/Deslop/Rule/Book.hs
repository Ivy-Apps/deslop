...
Everything here has already been through "Deslop.Rule.Book.Compiler". A value of
...
caller - which is what lets "Deslop.Rule.Enforcer" read it without a single
check of its own.

This module is deliberately free of IO and of the YAML shape. Reading rulebooks
off disk is "Deslop.Rule.Book.Loader"; what a rulebook file looks like is
"Deslop.Rule.Book.Dto".
-}
module Deslop.Rule.Book (
    Rulebook (..),
    RulebookId (..),
    Rule (..),
    RuleId (..),
